  ...
  extendNodeType () {
    return {
      ...
      excerpt: { // <!-- more -->
        type: GraphQLString,
        resolve: node => {
          let excerptpos = nodecopy.content.indexOf('<!-- more -->');
          if (excerptpos == -1) {
            excerptpos = nodecopy.content.indexOf('\n\n');
          }
          if (excerptpos == -1) {
            excerptpos = nodecopy.content.indexOf('\r\n\r\n');
          }
          let nodecopy = Object.assign({}, node);
          nodecopy.content = nodecopy.content.slice(0, excerptpos);
          return this._nodeToHTML(nodecopy);
        }
      },
      headings: {
