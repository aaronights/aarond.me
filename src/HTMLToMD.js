...
var td = new require('turndown')({headingStyle: 'atx', blankReplacement (content, node) {
...
function getPage(inputname) {
	...
	var meta = {
		title: '"' + $('h2.titlelink').html().replace("&apos;", "'").replace(/\.$/, "") + '"',
		date: $("meta[name='live_date']").attr("content"),
		author: "Aaron Davies",
		basename: $("meta[property='og:url']").attr("content").split(".")[0]
	}
	...
	$('.articletags a').each(function() {
		...
		if (tag != "Uncategorized" && tag != "uncategorized") {
			meta.tags.push(tag.split(" ").join("-").toLowerCase());
		}
	});
	...
	var page = td.turndown($('.entry').html()).trim().replace(/  +/g, ' ');
	...
	page = page.split('<iframe').join('<div class="youtube">\n	<iframe').split('</iframe>').join('</iframe>\n</div>');
	...
	page = page.replace("franchise got here. [![](../../media/images/blog/Rangers5.jpg)](/assets/static/src/media/images/blog/Rangers5.jpg)Power Rangers", "franchise got here.\n\n[![](../../media/images/blog/Rangers5.jpg)](/assets/static/src/media/images/blog/Rangers5.jpg)\n\nPower Rangers");
